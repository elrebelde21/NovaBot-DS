const { isUrl } = require('../libs/fuctions.js'); 
const { Tiktok } = require('../libs/tiktok.js'); 

let handler = async (message, { args, prefix, command }) => {
if (!args[0]) return message.reply(`Ejemplo:\n${prefix + command} https://vm.tiktok.com/ZMjdrFCtg/`);
if (!isUrl(args[0]) && !args[0].includes('tiktok')) return message.reply(`Link inválido!!`);

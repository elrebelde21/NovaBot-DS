...
El bot se encuentra en desarrollo, por lo que puede presentar errores o fallos de comandos. Puedes reportarlo con el comando '.report'. También puedes dar ideas o sugerencias sobre comandos o cosas que te gustaría que agregáramos al bot. entra al grupo de soporte [aqui](https://discord.gg/7gbAf4Pq7e).

> Selecciona una categoría abajo para ver sus comandos.\n`)
      .setFooter({ text: "NovaBot-DS • SkyUltraPlus", iconURL: imagenPP });

    // 🔑 FIX CLAVE AQUÍ
    const sent = message.content?.startsWith("/")
    ...
    collector.on("collect", async i => {
      ...
      if (comandos.length === 0) {
        return i.reply({
          content: "❌ No hay comandos en esta categoría aún.",
          ephemeral: true,
        });
      }
...
handler.help = ["menu"];
handler.tags = ["main"];
handler.desc = ["Mostrar el menú principal del bot"];
handler.slash = { name: "menu", description: "Mostrar el menú principal del bot" };

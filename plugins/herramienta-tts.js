...
const handler = async (message, { args, prefix, command }) => {
  try {
    ...
    if (!args.length && !message.reference)
      return message.reply(`*Uso:* ${prefix + command} <voz|idioma> <texto>\n\n*Voces:* anonymous, robot, grave, aguda, niño, demonio\n*Idiomas:* es, en, pt, fr...\n\nEjemplo:\n${prefix + command} anonymous Hola\n${prefix + command} es hello`);
    ...
    if (!text) return message.reply("⚠️ Escribe un texto para convertir a voz.");
    ...
    const resource = createAudioResource(ogg);

    connection.subscribe(player);
    player.play(resource);

    player.on(AudioPlayerStatus.Playing, () => {
      message.channel.send("🎶 **Reproduciendo tu TTS...**");
    });

    player.on(AudioPlayerStatus.Idle, () => {
      const conn = getVoiceConnection(message.guild.id);
      if (conn) conn.destroy();
      message.channel.send("🔚 **Audio finalizado.**");
    ...
    player.on("error", err => {
      ...
      message.reply("❌ Error al reproducir el audio.");
    });

  } catch (e) {
    console.error("❌ Error TTS:", e);
    return message.reply("❌ Error al generar voz.");
  }
};

const handler = async (message) => {
    ...
    const time = user.lastwork + 600000;
    
    if (new Date() - user.lastwork < 600000) {
        return message.reply(`*Estás cansado, debes descansar por lo menos ${msToTime(time - new Date())} para volver a trabajar!*`);
    }

    const trabajo = pickRandom(global.work);
    message.reply(`🛠 ${trabajo} ${hasil} XP`);
    user.exp += hasil;
